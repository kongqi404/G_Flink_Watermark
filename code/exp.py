...
class EventGeneratorMapFunction(MapFunction):
    def __init__(
        ...
        late_event_fraction,
        severe_lateness_upper_bound_ms,
    ):
        self.max_out_of_orderness_ms = max_out_of_orderness_ms  # 正常乱序的最大时长
        self.late_event_fraction = (
            late_event_fraction  # 迟到数据（超过 Watermark）的比例
        )
        self.severe_lateness_upper_bound_ms = (
            severe_lateness_upper_bound_ms  # 严重迟到的上限
        )
    ...
    def map(self, value):
        ...
        if random.random() < self.late_event_fraction:
            ...
            extra_delay = self.max_out_of_orderness_ms + random.randint(
                0, self.severe_lateness_upper_bound_ms
            )
...
class WindowResultProcessFunctionImpl(ProcessWindowFunction):
    def __init__(self, configured_watermark_delay_ms):
        ...
        self.configured_watermark_delay_ms = (
            configured_watermark_delay_ms  # 保存配置的水位线延迟参数
        )
    ...
    def process(self, key, context, elements: Iterable[EventAccumulator]):
        ...
        trigger_lag_ms = (
            max(0, current_watermark - window_end) if current_watermark >= 0 else -1
        )
        # 获取当前系统处理时间
...
class LateEventProcessFunctionImpl(ProcessFunction):
    ...
    def process_element(self, value: UserEvent, ctx):
        ...
        lateness = (
            (current_watermark - value.event_time) if current_watermark >= 0 else -1
        )  # type: ignore
        # 构造并输出迟到指标对象
...
def run_job(args):
    ...
    if enable_checkpointing:
        ...
        env.get_checkpoint_config().set_min_pause_between_checkpoints(
            int(checkpoint_interval_ms / 2)
        )
        env.get_checkpoint_config().set_checkpoint_timeout(
            max(checkpoint_interval_ms * 2, 120000)
        )

    # 定义 Watermark 策略
    watermark_strategy = WatermarkStrategy.for_bounded_out_of_orderness(
        Duration.of_millis(watermark_delay_ms)
    ...
    raw_events = raw_stream.map(
        EventGeneratorMapFunction(
            max_out_of_orderness_ms, late_event_fraction, severe_lateness_upper_bound_ms
        )
    ).name("synthetic-out-of-order-source")

    # 为数据流分配时间戳和 Watermark
    events_with_timestamps = raw_events.assign_timestamps_and_watermarks(
        watermark_strategy
    ...
    if "sliding" == window_type.lower():
        windowed_stream = keyed_stream.window(
            SlidingEventTimeWindows.of(
                Time.milliseconds(window_size_ms), Time.milliseconds(window_slide_ms)
            )
        )
    else:
        windowed_stream = keyed_stream.window(
    ...
    window_results = windowed_stream.aggregate(
        EventAggregateFunctionImpl(),
        WindowResultProcessFunctionImpl(watermark_delay_ms),
    ).name("window-aggregation")

    # 获取侧输出流中的迟到数据
    late_events = window_results.get_side_output(late_output_tag)
    # 对迟到数据进行处理，计算迟到指标
    late_metrics = late_events.process(LateEventProcessFunctionImpl()).name(
        "late-event-metrics"
    )
    ...
    if output_dir:
        # 创建 FileSink
        sink = (
            FileSink.for_row_format(output_dir, Encoder.simple_string_encoder("UTF-8"))
        ...
        window_results.map(lambda x: str(x), output_type=Types.STRING()).sink_to(
            sink
        ).name("window-results-file")
    else:
    ...
    if late_output_dir:
        late_sink = (
            FileSink.for_row_format(
                late_output_dir, Encoder.simple_string_encoder("UTF-8")
            )
            .with_output_file_config(
                OutputFileConfig.builder()
                .with_part_prefix("late-metric")
        ...
        # 将迟到指标转换为字符串并写入文件
        late_metrics.map(lambda x: str(x), output_type=Types.STRING()).sink_to(
            late_sink
        ).name("late-metrics-file")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Flink Watermark Latency Analysis in Python"
    )
    parser.add_argument(
        "--watermarkDelayMs", dest="watermark_delay_ms", type=int, default=5000
    )
    parser.add_argument(
        "--windowSizeMs", dest="window_size_ms", type=int, default=10000
    )
    parser.add_argument(
        "--windowSlideMs", dest="window_slide_ms", type=int, default=10000
    )
    parser.add_argument(
        "--allowedLatenessMs", dest="allowed_lateness_ms", type=int, default=0
    )
    parser.add_argument(
        "--windowType", dest="window_type", type=str, default="tumbling"
    )
    parser.add_argument(
        "--eventsPerSecond", dest="events_per_second", type=int, default=500
    )
    parser.add_argument(
        "--runDurationMs", dest="run_duration_ms", type=int, default=60000
    )
    parser.add_argument(
        "--maxOutOfOrdernessMs", dest="max_out_of_orderness_ms", type=int, default=5000
    )
    parser.add_argument(
        "--lateEventFraction", dest="late_event_fraction", type=float, default=0.2
    )
    parser.add_argument(
        "--severeLatenessUpperBoundMs",
        dest="severe_lateness_upper_bound_ms",
        type=int,
        default=5000,
    )
    parser.add_argument("--parallelism", dest="parallelism", type=int, default=6)
    parser.add_argument(
        "--autoWatermarkIntervalMs",
        dest="auto_watermark_interval_ms",
        type=int,
        default=200,
    )
    parser.add_argument(
        "--enableCheckpointing", dest="enable_checkpointing", type=bool, default=False
    )
    parser.add_argument(
        "--checkpointIntervalMs", dest="checkpoint_interval_ms", type=int, default=60000
    )
